...
def saturate_params(q_start, q_end, X, U, t1, t2):
    ...
    for i in range(0, D / 2):
        ...
        if np.abs(dt * a1 + v1)>v_max:
            a1 = np.sign(a1)*(v_limit - np.abs(v1)) / dt * 0.99

        var_min, var_max = X[i]
        vel_min, vel_max = X[i + D / 2]
        accel_min, accel_max = U[i]
        # for the case of bang bang
        while True:
            q_sat[i] = p1 + 0.5 * a1 * dt ** 2 + dt * v1
            q_sat[i + D / 2] = v1 + a1 * dt

            if q_sat[i] > var_max or q_sat[i + D / 2] > vel_max: # pos
                a1 -= 0.01 * a_max
                if a1<=accel_min:
                    break
            elif q_sat[i] < var_min or q_sat[i + D / 2] < vel_min:
                a1 += 0.01 * a_max
                if a1>=accel_max:
                    break
            else:
